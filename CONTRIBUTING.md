...
- **Succinct wording is a separate pass from non-duplication.** Removing an incident narrative (previous principle) doesn't by itself make the remaining rule read tightly: the rule-plus-reasoning that's left can still be padded, restating the same point twice in different words, spelling out something a shorter phrase already conveys, explaining an example instead of just naming it. After writing a bullet, reread it against the density of what surrounds it: a bullet in a list of one-liners should read close to one line unless it's genuinely more complex than its neighbors, not because it wasn't reread. Cut a clause if removing it costs no actual scope or clarity
