...
**Commit message**: descriptive but succinct, matching this repo's existing style, state the version reached (e.g. "Release 0.3.0"); a short body on what's in it is welcome, not required. There's no build or compile step here: `main` is the artifact, so a commit is effectively already live the moment it lands, write the message with that in mind rather than deferring detail to a release-notes step that doesn't exist.
