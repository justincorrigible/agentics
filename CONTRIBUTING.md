...
**Commit message**: subject is a declarative one-line summary of the release's most significant change, not a version stamp ("Release X.Y.Z"). The version number is internal machinery, what the upstream-check compares a `synced` tag against, not something a contributor scanning `git log` day to day needs restated; `CHANGELOG.md` § Released already owns that number exclusively (§ Versioning above), repeating it in the subject is a second copy of the same fact. Body is a verb-first list of the rest of what actually changed in substance, adds X, removes Y, improves Z, not how the release was assembled: don't describe relocating `CHANGELOG.md` entries or confirming the rollup, that's process, not content, and skip relative day words ("today," "earlier"), which mean nothing once committed. There's no build or compile step here: `main` is the artifact, so a commit is effectively already live the moment it lands; the message doubles as the release notes.
