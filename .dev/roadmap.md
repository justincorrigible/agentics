...
- Update propagation: design and implement a mechanism for agents to periodically check this repo for convention updates and offer to apply them with user permission; key design questions: trigger (cron vs. session-start), what counts as an update, permission granularity, breaking vs. additive handling
...
- **Context sync across projects:** as the template evolves and projects adopt it at different times, keeping project-specific files loosely aligned with the template is ongoing friction; Phase 2 (update propagation) addresses this structurally, but interim guidance on the "source of truth" ownership model may be useful: which conventions are global, which are template, which are project-specific, and how to reason about overlap
