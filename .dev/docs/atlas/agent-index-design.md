...
Design spec from a real, live design conversation (2026-08-08 through 2026-08-10), not yet a shipped convention. Supersedes the earlier family-registry prototype described in `interagent-communication-findings.md`: workspace becomes a field on one global index instead of a second, workspace-scoped file. Intended target: agentics 0.14.0, once tested and refined locally against real sessions.
...
Local prototype, not yet built. Next step: stand up the actual file and test the full loop (register, exact lookup, fuzzy request, validated callback, cleanup, staleness surfacing) against real sessions before converting any of this into `template/` convention text.
